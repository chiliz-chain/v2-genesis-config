...
/// StakingOverride contract used to bypass the local tx simulation issue of foundry.
/// This is basically Staking from v2.3.0 with an empty initNewParams() function added to it
/// and is used by SherlockHighRisk2.s.sol

contract StakingOverride is InjectorContextHolder {
    ...
    function initNewParams() public {}

    function getValidatorDelegation(address validatorAddress, address delegator) external view returns (
    ...
    function delegate(address validatorAddress) payable external {
        _delegateTo(msg.sender, validatorAddress, msg.value);
    }

    function undelegate(address validatorAddress, uint256 amount) external {
    ...
    function activateValidator(address validator) external onlyFromGovernance virtual {
    ...
    function disableValidator(address validator) external onlyFromGovernance virtual {
    ...
    function deposit(address validatorAddress) external payable onlyFromCoinbaseOrTokenomicsOrStakingPool onlyZeroGasPrice virtual {
    ...
    function getValidatorFee(address validatorAddress) external view returns (uint256) {
        // make sure validator exists at least
        Validator memory validator = _validatorsMap[validatorAddress];
    ...
    function getValidatorFeeAtEpoch(address validatorAddress, uint64 beforeEpoch) external view returns (uint256) {
        // make sure validator exists at least
        Validator memory validator = _validatorsMap[validatorAddress];
        if (validator.status == ValidatorStatus.NotFound) {
            return 0;
    ...
    function getPendingValidatorFee(address validatorAddress) external view returns (uint256) {
        // make sure validator exists at least
        Validator memory validator = _validatorsMap[validatorAddress];
        if (validator.status == ValidatorStatus.NotFound) {
            return 0;
    ...
    function claimValidatorFee(address validatorAddress) external {
        // make sure validator exists at least
        Validator storage validator = _validatorsMap[validatorAddress];
    ...
    function claimValidatorFeeAtEpoch(address validatorAddress, uint64 beforeEpoch) external {
        // make sure validator exists at least
        Validator storage validator = _validatorsMap[validatorAddress];
        // only validator owner can claim deposit fee
        require(validator.status != ValidatorStatus.NotFound, "nf"); // not found
    ...
    function getDelegatorFee(address validatorAddress, address delegatorAddress) external view returns (uint256) {
        return _calcDelegatorRewardsAndPendingUndelegates(validatorAddress, delegatorAddress, _currentEpoch(), true);
    }

    function getDelegatorFeeAtEpoch(address validatorAddress, address delegatorAddress, uint64 beforeEpoch) external view returns (uint256) {
        return _calcDelegatorRewardsAndPendingUndelegates(validatorAddress, delegatorAddress, beforeEpoch, true);
    }

    function getPendingDelegatorFee(address validatorAddress, address delegatorAddress) external view returns (uint256) {
        return _calcDelegatorRewardsAndPendingUndelegates(validatorAddress, delegatorAddress, _nextEpoch(), true);
    }

    function claimDelegatorFee(address validatorAddress) external {
    ...
    function claimDelegatorFeeAtEpoch(address validatorAddress, uint64 beforeEpoch) external {
    ...
    function slash(address validatorAddress) external onlyFromSlashingIndicator virtual {
